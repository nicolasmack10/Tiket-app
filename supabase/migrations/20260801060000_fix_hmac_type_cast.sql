-- pgcrypto's hmac(bytea, bytea, text) couldn't resolve with a bare string
-- literal for the algorithm name ("function hmac(bytea, bytea, unknown) does
-- not exist") — casting it explicitly to text fixes the overload resolution.
...
begin
  ...
  v_sig := encode(hmac(convert_to(v_msg, 'UTF8'), convert_to(ticket_hmac_secret(), 'UTF8'), 'sha256'::text), 'hex');
...
begin
  ...
  v_msg := p_ticket_id || '|' || v_order_id::text || '|' || v_event_name || '|' || v_tier_name;
  v_expected := encode(hmac(convert_to(v_msg, 'UTF8'), convert_to(ticket_hmac_secret(), 'UTF8'), 'sha256'::text), 'hex');
