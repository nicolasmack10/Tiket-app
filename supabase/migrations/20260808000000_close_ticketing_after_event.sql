-- Rien n'empêchait d'acheter un billet pour un événement déjà passé : la
-- billeterie doit se fermer une fois la date/heure de l'événement atteinte.
-- La comparaison se fait en heure d'Afrique centrale (Congo), pas en UTC brut,
-- pour correspondre à l'heure réelle affichée aux organisateurs/clients.
...
begin
  ...
  select tiers, name, require_buyer_name, date, time
    into v_tiers, v_event_name, v_require_name, v_date, v_time
  from events where code = p_event_code for update;
  if v_tiers is null then
    raise exception 'Événement introuvable';
  end if;

  if (v_date || ' ' || coalesce(v_time, '00:00'))::timestamp at time zone 'Africa/Brazzaville' < now() then
    raise exception 'La billetterie de cet événement est fermée — l''événement a déjà eu lieu';
  end if;
