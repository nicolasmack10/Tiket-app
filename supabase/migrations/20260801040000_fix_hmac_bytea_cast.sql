-- pgcrypto's hmac() takes bytea, not text — the previous migration passed text
-- directly and errored ("function hmac(text, text, unknown) does not exist").
-- Using convert_to(..., 'UTF8') rather than a bare ::bytea cast, since a plain
-- cast parses the string as bytea's escape format (backslashes in an event or
-- buyer name would be misinterpreted); convert_to always does a clean UTF8
-- byte encoding regardless of content.
...
begin
  ...
  v_sig := encode(hmac(convert_to(v_msg, 'UTF8'), convert_to(ticket_hmac_secret(), 'UTF8'), 'sha256'), 'hex');
...
begin
  ...
  v_msg := p_ticket_id || '|' || v_order_id::text || '|' || v_event_name || '|' || v_tier_name;
  v_expected := encode(hmac(convert_to(v_msg, 'UTF8'), convert_to(ticket_hmac_secret(), 'UTF8'), 'sha256'), 'hex');
